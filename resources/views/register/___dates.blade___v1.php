                    ...
                    <div id="inputs-{{ $item }}-{{ $i }}">
                      <div class="col-xs-11 col-xs-offset-1 no-padding margin-bottom-15px">
                        @if ($i == 0)
                          ...
                          <div class="col-xs-1 no-padding">
                              ...
                              @if ( $errors->has('day.'.$item.'.'.$i.'.from') || $errors->has('day.'.$item.'.'.$i.'.to') )
                                <button class="btn btn-link btn-invert-red" data-switch="true" data-enable="#{{ $item.'_'.$i }}_from,#{{ $item.'_'.$i }}_to" data-enabled="true">
                                  <span class="fa fa-lock register-icon-lock"></span>
                                </button>
                              @else
                                <button class="btn btn-link btn-invert-red" data-switch="true" data-enable=".day-{{ $item }}-from,.day-{{ $item }}-to,#add-element-{{$item}},.delete-element-{{ $item }}" data-enabled="false">
                        ...
                        <div class="col-xs-3 {{ ($i > 0) ? 'col-xs-offset-4' : '' }} no-padding">
                          {{ Form::text('day['. $item .']['.$i.'][from]', null, ['id' => $item.'_'.$i.'_from', 'class' => 'day-'.$item.'-from form-control text-center datetimepicker-time' . ($errors->has('day.'.$item.'.'.$i.'.from') ? ' has-error' : ''), 'placeholder' => '--:-- --', 'required' => 'required']) }}
                        </div>
                        <div class="col-xs-1">
                          <p class="text-center">A</p>
                        </div>
                        <div class="col-xs-3 no-padding">
                          {{ Form::text('day['. $item .']['.$i.'][to]', null, ['id' => $item.'_'.$i.'_to', 'class' => 'day-'.$item.'-to form-control text-center datetimepicker-time' . ($errors->has('day.'.$item.'.'.$i.'.to') ? ' has-error' : ''), 'placeholder' => '--:-- --', 'required' => 'required']) }}
                        </div>
                        <div class="col-xs-1 no-padding">
                          @if ($i == 0)
                            <button id="add-element-{{ $item }}" class="btn btn-link btn-invert-red app-add-element" data-container="#{{ 'inputs-container-' . $item_index  }}" data-item="{{ $item }}" disabled="disabled">
                              <span class="glyphicon glyphicon-plus"></span>
                            </button>
                          @else 
                            <button data-delete="#inputs-{{ $item }}-{{ $i }}" class="app-delete-element delete-element-{{ $item }} btn btn-link btn-invert-red">
